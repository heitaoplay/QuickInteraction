...
采用与 Razor Wings 相同的架构：Loader 只是一个极简外壳（约 1KB），真正自包含的脚本 `assets/main.js`（已内联 bcModSdk + 全部业务逻辑）由 GitHub Actions 在每次 push 时自动构建并部署到 **GitHub Pages**。Loader 每次刷新页面通过 `import()` 拉取最新构建，所以**刷新即更新**，稳定且不依赖油猴轮询、不踩 raw 重定向。
